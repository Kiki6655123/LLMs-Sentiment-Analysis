...
# 设置日志
log_file = f'/gemini/code/evaluate_deepseek_qwen_7b_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
logging.basicConfig(filename=log_file, level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s', encoding='utf-8')
logging.info("开始评估 DeepSeek-R1-Distill-Qwen-7B 和微调模型对比")
...
def load_model_and_tokenizer(base_path, finetuned_path=None):
    try:
        ...
        base_model = Qwen2ForSequenceClassification.from_pretrained(
            base_path, num_labels=2, ignore_mismatched_sizes=True  # 恢复二分类
        ).to(device)
...
def evaluate_and_plot(trainer, dataset, name):
    ...
    logging.info(f"{name} 评估结果: {preds}")
    print(f"{name} 评估结果:", preds)
...
if __name__ == "__main__":
    ...
    finetuned_model_path = "/gemini/code/finetuned_deepseek_qwen_7b"
